...
describe('calling favorite reducer', () => {
  ...
  it('should add a gif object to the favorites ADD_FAVORITE_GIF', ()=>{
    const action = favoriteActions.addFavoriteGif(gif);
  ...
  it('should remove a gif object to the favorites REMOVE_FAVORITE_GIF', ()=>{
    const gifId=gif.id;
    const action = favoriteActions.removeFavoriteGif(gifId);
    const newState = favoriteReducer(gifs, action);

    const result=Object.keys(newState);
    const hasGiphy=result.filter(key => key === gifId);
  ...
  it('should set isFavorite to true SET_FAVORITE_GIF', ()=>{
    ...
    const action = favoriteActions.setFavoriteGif(favoriteGif);
    const newState = favoriteReducer(gifs, action);
